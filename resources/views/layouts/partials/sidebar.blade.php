{{--
    ✅ SIDEBAR — Bootstrap 4 + SB Admin 2
    Pakai data-toggle, data-target, data-parent (Bootstrap 4)
    BUKAN data-bs-toggle (itu Bootstrap 5)
--}}
    ...
    @canany(['categories.index', 'books.index', 'members.index'])
        ...
        <li class="nav-item {{ request()->is('categories*') || request()->is('books*') || request()->is('members*') ? 'active' : '' }}">
            <a class="nav-link {{ request()->is('categories*') || request()->is('books*') || request()->is('members*') ? '' : 'collapsed' }}"
                ...
                aria-expanded="{{ request()->is('categories*') || request()->is('books*') || request()->is('members*') ? 'true' : 'false' }}"
    ...
    @can('loans.index')
        ...
        <li class="nav-item {{ request()->is('loans*') || request()->is('loan-extensions*') ? 'active' : '' }}">
            <a class="nav-link {{ request()->is('loans*') || request()->is('loan-extensions*') ? '' : 'collapsed' }}"
                href="#"
                data-toggle="collapse"
                data-target="#collapseLoans"
                aria-expanded="{{ request()->is('loans*') || request()->is('loan-extensions*') ? 'true' : 'false' }}"
            ...
            <div id="collapseLoans"
                ...
                data-parent="#accordionSidebar">
                <div class="bg-white py-2 collapse-inner rounded">
                    <a class="collapse-item {{ request()->is('loans') && !request()->is('loan-extensions*') ? 'active' : '' }}"
                        href="{{ route('loans.index') }}">Data Peminjaman</a>
